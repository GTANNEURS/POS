...
import { Button, EmptyState, Field, Input, LoadingBlock, PageHeader, SectionCard, Textarea } from "../../components/ui/primitives";
...
type Supplier = {
  ...
  purchases: Array<{ totalAmount: number }>;
};
...
export function SuppliersPage() {
  ...
  async function load(searchValue = "") {
    ...
    setError(null);
    try {
      setItems(await api<Supplier[]>(`/suppliers${searchValue ? `?search=${encodeURIComponent(searchValue)}` : ""}`));
    } catch (err) {
  ...
  function openDetails(id: string) {
    navigate(`/gestion/fournisseurs/${id}`);
  }
  ...
  if (loading && items.length === 0) return <LoadingBlock label="Chargement des fournisseurs..." />;

  return (
    <>
      <div className="space-y-6">
        <PageHeader
          eyebrow="Gestion des Fournisseurs"
          title=""
          actions={
            <>
        ...
        <SectionCard
          title="Liste"
          actions={<Input className="w-full min-w-[260px] md:w-[320px]" placeholder="Rechercher un fournisseur..." value={search} onChange={(event) => { const value = event.target.value; setSearch(value); void load(value); }} />}
        >
          ...
          {items.length === 0 ? (
            <EmptyState
              ...
              description="Ajoute un premier fournisseur pour demarrer le cycle achat."
              compact
              action={<Button className="!h-9 !px-3.5 !text-[13px]" onClick={openCreateModal}>Nouveau fournisseur</Button>}
            />
          ) : (
            <div className="table-wrap">
              <table className="data-table">
                ...
                <tbody>
                  {items.map((supplier) => {
                    const spent = supplier.purchases.reduce((sum, purchase) => sum + Number(purchase.totalAmount), 0);
                    return (
                      <tr
                        key={supplier.id}
                      ...
                      >
                        <td>
                          ...
                          <div className="mt-1 text-xs text-[#b9aa9b]">{supplier.address || supplier.notes || "Aucune precision"}</div>
                        </td>
                        <td>
                          <div>{supplier.phone || "-"}</div>
                          <div className="mt-1 text-xs text-[#b9aa9b]">{supplier.email || "-"}</div>
                        </td>
                        <td>{supplier.city || "-"}</td>
                        <td>{formatNumber(supplier.purchasesCount)}</td>
